...
단, 사용자 요청이나 `[평가 필수 전략]`이 Playwright 클릭, 상태 기반 wait 등 특정 구현을
요구하면 그 제약이 URL/API 최적화보다 우선입니다. API를 발견해도 필수 구현 방식을
대체하지 말고 참고 정보로만 Blueprint에 기록하세요.
...
3. URL 직접 순회가 가능하면 버튼 클릭이나 Playwright 반복보다 Requests/HTTP 클라이언트
   기반 수집 전략을 우선 권장하세요. 단, 명시된 필수 전략과 충돌하면 권장하지 마세요.
